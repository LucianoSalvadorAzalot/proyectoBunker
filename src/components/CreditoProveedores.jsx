...
useEffect(() => {
 ...
 const generarPDFOrdenCobro = (totalPago) => {
  ...
  // Datos de la empresa
  const empresaNombre = "Deco Artesanias";
  const empresaDireccion = "Entre rios 213 - San Miguel de Tucuman - C.P: (4000)";
  ...
  // Logo de la empresa
  try {
    doc.addImage(logoticket, "PNG", 10, 10, 50, 20);
  } catch (error) {
    console.warn(
  ...
  doc.autoTable({
    ...
    theme: "grid",
    headStyles: {
      fillColor: [100, 150, 200], // azul claro
            ...
            <tbody>
              {resultado.slice(primerIndex, ultimoIndex).map((proveedor) => (
                <tr key={proveedor.Id_proveedor}>
                  ...
                  <td>
                  <Button onClick={() => obtenerDetalleProveedorCompra(proveedor.Id_proveedor)} style={{ backgroundColor: '#244983', color: 'white' }}>
